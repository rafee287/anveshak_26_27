...
#     # --- TOP 50% FALLBACK LOGIC ---
#     if not candidates:
#         print("No contours found in scaled ROI. Checking Top 50% Fallback...")
#         top_half = orig_gray[0:(oh//2), 0:ow]
        
#         # Inverted thresholding at 75 (darker than 75 becomes white/255)
#         _, fb_binary = cv2.threshold(top_half, 75, 255, cv2.THRESH_BINARY_INV)
...
#         # Drawing the raw contour directly onto result_vis (left unchanged from original logic)
...
def detect_arrow(image_path):
    ...
    # --- CHANGED: SAVE A PURE COPY BEFORE THE BLUR DESTROYS IT ---
    ...
    # --- BOTTOM 50% FALLBACK LOGIC ---
    if not candidates:
        print("No contours found in scaled ROI. Checking Bottom 50% Fallback...")
        offset_y = int(oh * 0.6)
        
        # --- CHANGED: USE THE UN-MUTATED RAW IMAGE HERE ---
    ...
    # --- CHANGED: DRAW ON THE PURE IMAGE, NOT THE MUTATED ONE ---
