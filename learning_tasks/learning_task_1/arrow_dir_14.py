...
# --- LOGIC 2: CONTOUR LENGTH / POINT COUNT ---
def detect_direction_from_contours(candidates, is_fallback=False):
    ...
    # Calculate the "length" based on number of contour perimeter points
    left_len = len(left_pts)
    right_len = len(right_pts)

    print(f"Left Contour Length: {left_len} pts | Right Contour Length: {right_len} pts")
...
if __name__ == "__main__":
    print("Starting Batch Processing Pipeline (Contour Point Logic)...")
    ...
    fig.canvas.manager.set_window_title('Contour Length Arrow Detection Results')
